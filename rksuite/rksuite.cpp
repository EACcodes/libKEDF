...
void RKSUITE::ut(ODEKernel* kernel, double twant, double& tgot,
...
//C
      ...
      if (goback) {
         //std::string str ("Both solution and derivative");
         char * cstr = new char [29];
         std::strcpy (cstr, "Both solution and derivative");//str.c_str());
         intrp(twant,cstr,rkcom1.neqn,ygot,
            ypgot,kernel,work,&work[rkcom3.printp],rkcom3.lnintp);
         chkfl(ask,baderr);
         if (baderr) goto label100;
         tgot = twant;
      }
...
//C
   ct(kernel,tnow,ygot,ypgot,work,cflag);
   ier = cflag;
//C
...
void RKSUITE::ct(ODEKernel* kernel, double& tnow, double ynow[],
...
//C  and YPNOW(*).
      kernel->evaluate(rkcom2.t,&work[rkcom3.pry],&work[rkcom3.pryp]);
      rkcom2.nfcn++;
      for (l = 0; l < rkcom1.neqn; l++) {
...
//C  stiffness.
      stiff(kernel,havg,jflstp,toomch,maxfcn,work,ier,nrec);
//C
...
//C
   main = true;
   htry = rkcom2.h;
   step(kernel,rkcom1.neqn,rkcom2.t,&work[rkcom3.pry],&work[rkcom3.pryp],&work[rkcom3.prstgs],rkcom1.tolr,htry,
...
//C
   if (rkcom6.erason) {
      truerr(kernel,rkcom1.neqn,&work[rkcom3.pry],rkcom1.tolr,&work[rkcom3.prwt],&work[rkcom6.przy],
...
void RKSUITE::intrp(double twant, char reqest[], int nwant, double ywant[], double ypwant[],
   ODEKernel* kernel, double work[], double wrkint[],
   int lenint)
{
//C$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
//C
...
//C
      if (rkcom4.methd == 1) {
         formi(kernel,rkcom1.neqn,nwant,&work[rkcom3.pry],&work[rkcom3.pryp],&work[rkcom3.pryold],
...
void RKSUITE::formi(ODEKernel* kernel, int neq, int nwant, double y[], double yp[],
...
//C
         for (i = 9; i <= 11; i++) {
            ...
            if (i == 9) {
               kernel->evaluate(rkcom2.told+rkcom4.c[i]*rkcom2.hold,p,
                  &stages/*[1][7]*/[neq*(7-1)]);
               rkcom2.nfcn++;
            }
            else if (i == 10) {
               kernel->evaluate(rkcom2.told+rkcom4.c[i]*rkcom2.hold,p,xstage);
               rkcom2.nfcn++;
            }
            else {
               kernel->evaluate(rkcom2.told+rkcom4.c[i]*rkcom2.hold,p,
...
void RKSUITE::truerr(ODEKernel* kernel, int neq, double y[],
...
//C
//C  Take a step.
      step(kernel,neq,tsec,zy,zyp,zstage,ztest1,hsec,weight,zynew,
...
void RKSUITE::step(ODEKernel* kernel, int neq, double tnow,
...
//C
      ...
      kernel->evaluate(tstg,ynew,/* stages[1][rkcom4.ptr[i]] */ &stages[neq*(rkcom4.ptr[i]-1)]);
//C
...
void RKSUITE::stiff(ODEKernel* kernel, double havg, int& jflstp,
...
//C
      stiffa(kernel,rkcom2.t,&work[rkcom3.pry],rkcom2.h,havg,rkcom1.tnd,maxfcn,&work[rkcom3.prwt],
...
void RKSUITE::stiffa(ODEKernel* kernel,double x, double y[],
...
//C
   ...
   label60:
//C
   stiffd(v0,havg,x,y,kernel,fxy,wt,scale,v0v0,v1,v1v1,vtemp);
//C
...
//C
   ...
   stiffd(v1,havg,x,y,kernel,fxy,wt,scale,v1v1,v2,v2v2,vtemp);
...
//C  of eigenvalues.
//C
   stiffd(v2,havg,x,y,kernel,fxy,wt,scale,v2v2,v3,v3v3,vtemp);
...
void RKSUITE::stiffd(double v[], double havg, double x, double y[],
   ODEKernel* kernel, double fxy[], double wt[],
