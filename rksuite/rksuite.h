...
public:
    /**
     *  f - IN derivatives function (double tin, double yin[neq], double ypout[neq])
     */
    virtual void evaluate(const double t, double y[], double yp[]) = 0;
    virtual ~ODEKernel(){};
};
...
public:
    ...
    */
    ...
    @param uflag - OUT status, (1 - success, 2 - inefficient usage, 3 - work intensive,
        ...
        stdout.
    */
    void ut(ODEKernel* kernel, double twant, double& tgot,
        double ygot[], double ypgot[], double ymax[], int& uflag) {
        ut( kernel, twant, tgot, ygot, ypgot, ymax, work, uflag );
    }
    ...
    */
    ...
    @param cflag - OUT status, (1 - success, 2 - inefficient usage, 3 - work intensive,
        ...
        stdout.
    */
    void ct(ODEKernel* kernel, double& tnow, double ynow[],
        double ypnow[], int& cflag) {
            ct( kernel, tnow, ynow, ypnow, work, cflag );
    }
    ...
    */
    void intrp(double twant, char reqest[], int nwant, double ywant[], double ypwant[],
        ODEKernel* kernel) {
            ...
            intrp( twant, reqest, nwant, ywant, ypwant, kernel, work, wrkint, lenint );
    }
...
protected:
    ...
    void step(ODEKernel* kernel, int neq, double tnow,
    ...
    void stepb(int neq, double y[], double yp[], double h, double ynew[],
	    ...
	    double weight[]);
    void stiff(ODEKernel* kernel, double havg, int& jflstp,
	    bool toomch, int maxfcn, double work[], int& ier, int& nrec);
    void stiffa(ODEKernel* kernel,double x, double y[],
    ...
    void formi(ODEKernel* kernel, int neq, int nwant, double y[], double yp[],
	    double yold[], double ypold[], double stages/*[neq][]*/[], bool calstg,
    double xstage[], double p[]);
    void truerr(ODEKernel* kernel, int neq, double y[],
